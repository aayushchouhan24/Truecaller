...
export default function WhoSearchedScreen() {
  ...
  const fetchSearches = useCallback(async (p: number, reset = false) => {
    try {
      const res = await usersApi.getWhoSearchedMe(p);
    ...
    <TouchableOpacity
      ...
      onPress={() => {
        if (item.user?.phoneNumber) {
          router.push({ pathname: '/number-detail', params: { phoneNumber: item.user.phoneNumber } });
        }
      }}
    >
      <View style={s.avatar}>
        <Text style={s.avatarT}>{getInitials(item.user?.name)}</Text>
      </View>
      <View style={s.rowInfo}>
        <Text style={s.rowName} numberOfLines={1}>{item.user?.name || 'Unknown'}</Text>
        <Text style={s.rowPhone}>{item.user?.phoneNumber || ''}</Text>
      </View>
      <View style={s.rowRight}>
        <Ionicons name="search" size={14} color="#6B6B6B" />
        <Text style={s.rowTime}>{item.searchedAt ? timeAgo(item.searchedAt) : ''}</Text>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Who searched for me</Text>
      ...
      <View style={s.banner}>
        ...
        <Text style={s.bannerText}>See who has searched for your number on Truecaller. This is a Premium feature.</Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={s.center}>
          ...
          <Text style={s.emptySub}>When someone searches for your number, they{"'"}ll appear here.</Text>
        </View>
      ) : (
        <FlatList
          data={searches}
...
const s = StyleSheet.create({
  ...
  avatar: {
    ...
    backgroundColor: '#2A1B3D', justifyContent: 'center', alignItems: 'center',
  },
  avatarT: { color: '#CE93D8', fontSize: 16, fontWeight: '700' },
  ...
  rowTime: { color: '#6B6B6B', fontSize: 11 },
});
